...
boss = entity(W - 50, H - 50, 120, 120, 0, 0, 0, 1000)
...
def get_trajectory_points(start_x, start_y, vel_x, vel_y):
    ...
    for _ in range(10):
        ...
        y += vy
        vy += 0.40  
        if y >= floor_limit or x < 0 or x > W:
            break
    return points
...
def change_phase():
    global phase_boss, is_fireball_rotated, img, fireball_sprite
    if boss.health == 1000:
        phase_boss = 1
...
def boss_spikes():
    if boss_cool.b_attack <= 0 and phase_boss == 1:
        for i in range(1, 10):
            if i % 2 == 0:
                new_spike = entity(boss.pos_x + (40 * i), boss.pos_y + 40, 20, 200, 0, 0, 0, 1)
            else:
                new_spike = entity(boss.pos_x - (40 * i), boss.pos_y + 40, 20, 200, 0, 0, 0, 1)
            list_spike.append(new_spike)
            boss_cool.b_attack = randint(200, 200)
    if boss_cool.b_attack > 0:
        boss_cool.b_attack -= 1
    if phase_boss == 1:
        for spike in list_spike:
            spike.pos_y -= 8
            if spike.pos_y < boss.pos_y:
                list_spike.remove(spike)
            if hitbox(player, spike) == True:
                player.health = player.health - 10
                list_spike.remove(spike)
                continue
            if spike.pos_x < -40 and spike in list_spike:
                list_spike.remove(spike)
    if phase_boss == 2:
        list_spike.clear()
...
def consumable():
    if consumable_cool.a_attack <= 0:
        new_consumable = entity(randint(10,W - 40), 0, 40, 40, 0, 0, 0, 1,)
...
def draw_game():
    ...
    screen.blit(sprite_player, (player.pos_x - 40, player.pos_y - 80))
    screen.blit(img_sprite_boss, (boss.pos_x - 120, boss.pos_y - 120))
    if player_cool.plat > 180:
        pygame.draw.rect(screen, bleue, (plat.pos_x, plat.pos_y, plat.x, plat.y))
    pygame.draw.rect(screen, rouge, (100, 50, boss.health, 40))
    pygame.draw.rect(screen, vert, (140, 100, player.health * 8, 40))

    for ball in list_fireball:
        screen.blit(fireball_sprite, (ball.pos_x, ball.pos_y))
    for arrow in list_arrow:
        screen.blit(arrow_sprite, (arrow.pos_x, arrow.pos_y))
    for new_spike in list_spike:
        pygame.draw.rect(screen, marron,(new_spike.pos_x, new_spike.pos_y, new_spike.x , new_spike.y ))
    ...
    for i in range(1, len(points)):
        pygame.draw.line(screen, rouge, points[i-1], points[i], 2)
    
    for new_con in list_consumable:
        screen.blit(img_consumable, (new_con.pos_x, new_con.pos_y))
    for new_con in list_consumable_fixe:
        screen.blit(img_consumable, (new_con.pos_x, new_con.pos_y))
    ...
    if phase_boss == 2:
        ...
        screen.blit(shadow,(W//2 - title.get_width()//2 + 3,163))
